...
import { Activity, Eye, EyeOff, Github, Lock, Mail } from "lucide-react";
...
export function LoginForm({ nextPath, initialMode = "login" }: LoginFormProps) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#0B0F14] px-6 py-12 text-white">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-gradient-to-br from-gray-900/50 to-gray-800/50 p-8 backdrop-blur-sm">
        <h1 className="mb-2 text-center text-2xl font-bold">Create account</h1>
        <p className="mb-6 text-center text-sm text-gray-400">Create your AgentScope workspace</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block">
            <span className="mb-2 block text-sm font-medium">Display name</span>
            <input
              className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-purple-500 focus:outline-none"
        ...
        <p className="mt-6 text-center text-sm text-gray-400">
          ...
          <Link href="/login" onClick={() => setMode("login")} className="font-medium text-purple-400 hover:text-purple-300">
            Sign in
          </Link>
        </p>
      </div>
