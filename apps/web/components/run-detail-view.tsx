...
export function RunDetailView({
  ...
  const runModel = ordered.find((span) => span.model)?.model ?? ordered.find((span) => span.provider)?.provider ?? "gpt-4.1-mini";
    ...
    <section className="space-y-6 bg-[#0B0F1A] p-6 text-white">
      <header className="rounded-xl border border-white/10 bg-gradient-to-b from-[#0F1629] to-[#0B1220] p-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="text-5xl font-bold text-white">{liveRun.workflow_name || liveRun.agent_name}</h1>
        ...
        <p className="mt-3 text-lg text-gray-400">Primary Insight: {primaryInsight}</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[65fr_35fr]">
        <main className="space-y-0 rounded-xl border border-white/10 bg-gradient-to-b from-[#0F1629] to-[#0B1220] p-5">
          <h2 className="mb-4 text-3xl font-semibold text-white">Execution Story</h2>
          {storyItems.map((item, index) => (
            <div key={item.id} className="relative pl-8">
              {index < storyItems.length - 1 ? <div className="absolute left-[11px] top-6 h-full w-px bg-white/10" /> : null}

              {item.kind === "step" ? (
                <div className="pb-4">
                  <div className="flex items-center gap-3">
                    {item.stepType === "tool" ? (
                      <span className="size-3 rounded-[2px] bg-yellow-400" />
        ...
        <aside className="space-y-4 xl:sticky xl:top-6 xl:self-start">
          ...
          <section className="rounded-xl border border-white/10 bg-gradient-to-b from-[#0F1629] to-[#0B1220] p-4">
            <button type="button" onClick={() => setContextOpen((v) => !v)} className="flex w-full items-center justify-between text-left text-white">
              ...
              <ChevronDown className={`size-5 transition ${contextOpen ? "rotate-180" : ""}`} />
            </button>
            {contextOpen ? (
