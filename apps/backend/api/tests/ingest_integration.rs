...
#[sqlx::test(migrations = "../storage/migrations")]
async fn ingest_redacts_sensitive_values_but_preserves_payload_structure(pool: PgPool) {
    let project_id = seed_project(&pool, "redact-org", "redact-project").await;
    let org_id: String =
        sqlx::query_scalar("SELECT organization_id::text FROM projects WHERE id = $1::uuid")
            .bind(&project_id)
            .fetch_one(&pool)
            .await
            .unwrap();
    seed_user_with_role(&pool, &org_id, "redact-admin@example.com", "admin").await;
    seed_project_api_key(&pool, &project_id, TEST_API_KEY).await;

    let router = app(Storage { pool: pool.clone() }, jwt_settings());
    let token = login_token(&router, "redact-admin@example.com").await;
    ...
    let payload = IngestPayload {
        run: Run {
            ...
            session_id: None,
            environment: None,
            workflow_name: "redact-workflow".to_string(),
            agent_name: "redact-agent".to_string(),
        ...
        artifacts: vec![
            Artifact {
                ...
                payload: json!({
                    "messages": [
                        {"role": "user", "content": "Email me at alice@example.com and use card 4242 4242 4242 4242"}
                    ],
                    "system": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.abc.def"
                }),
            },
            Artifact {
                id: uuid::Uuid::new_v4().to_string(),
                ...
                kind: "log".to_string(),
                payload: json!({
                    "message": "Operator phone +1 (415) 555-2671 and ssn 123-45-6789",
                    "detail": "api key sk-1234567890abcdefghijklmnop"
                }),
            },
            Artifact {
                id: uuid::Uuid::new_v4().to_string(),
                ...
                payload: json!({"value": "leave me untouched"}),
            },
        ],
    };
    ...
    assert!(prompt_payload["system"]
        .as_str()
        .unwrap_or_default()
        .contains("[REDACTED_TOKEN]"));

    assert!(log_payload["message"]
        .as_str()
        .unwrap_or_default()
    ...
    assert!(log_payload["detail"]
        .as_str()
        .unwrap_or_default()
        .contains("[REDACTED_API_KEY]"));

    // Unrelated artifact kinds remain untouched.
    assert_eq!(tool_payload["value"], json!("leave me untouched"));
}
